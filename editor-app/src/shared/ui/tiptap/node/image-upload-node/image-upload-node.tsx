...
import { isValidPosition } from '@/shared/lib/tiptap'
import '@/shared/ui/tiptap/node/image-upload-node/image-upload-node.scss'
	...
	<>
		...
		<div className='tiptap-image-upload-content'>
			<span className='tiptap-image-upload-text'>
				<em>Нажмите для загрузки</em> или перетащите файлы сюда
			</span>
			<span className='tiptap-image-upload-subtext'>
				Максимум 1 файл, {maxSize / 1024 / 1024}мб.
			</span>
		</div>
	</>
)
...
export const ImageUploadNode: React.FC<NodeViewProps> = props => {
	...
	const handleUpload = async (file: File) => {
		...
		if (file.size > maxSize) {
			showCustomToast({
				description: `Файл не может быть больше ${maxSize / 1024 / 1024}мб`,
				type: 'error',
				title: 'Ошибка'
			})
			return
	...
	return (
		<NodeViewWrapper className='tiptap-image-upload' tabIndex={0} onClick={handleClick}>
			{isLoading ? (
				<div className='tiptap-image-upload-loader'>
					<Loader />
				</div>
			) : (
				<ImageUploadDragArea onFile={handleFileSelect}>
