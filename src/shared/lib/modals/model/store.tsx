	...
	return ({ close }) => {
		...
		return (
			<ModalShell
				...
				description={
					options.description ?? 'Вы уверены, что хотите продолжить?'
				}
				containerClassName={
					options.containerClassName ?? 'w-[400px] max-w-[92vw]'
				}
...
export const useModals = createGStore(() => {
	...
	const openElement = (element: ReactNode): string =>
		openRender(() => element as any)
...
export const modals = {
	...
	openElement: (element: ReactNode) =>
		useModals.getState().openElement(element),
	openContent: (options: ContentModalOptions) =>
		useModals.getState().openContent(options),
	openConfirm: (options: ConfirmModalOptions) =>
		useModals.getState().openConfirm(options),
	openDeleteModal: (options: DeleteModalOptions) =>
		useModals.getState().openDeleteModal(options),
